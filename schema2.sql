...
/*!50001 VIEW `franchise_monthly_purchase_mix` AS select `f`.`id` AS `franchise_id`,year(`so`.`created_at`) AS `year`,month(`so`.`created_at`) AS `month`,sum((case when (`so`.`warehouse_id` is not null) then (`soi`.`quantity` * `soi`.`unit_price`) else 0 end)) AS `internal_amount`,sum((case when (`so`.`supplier_id` is not null) then (`soi`.`quantity` * `soi`.`unit_price`) else 0 end)) AS `external_amount`,(case when (sum((`soi`.`quantity` * `soi`.`unit_price`)) > 0) then round(((sum((case when (`so`.`warehouse_id` is not null) then (`soi`.`quantity` * `soi`.`unit_price`) else 0 end)) / sum((`soi`.`quantity` * `soi`.`unit_price`))) * 100),2) else NULL end) AS `internal_percentage` from (((`stock_orders` `so` join `trucks` `t` on((`t`.`id` = `so`.`truck_id`))) join `franchises` `f` on((`f`.`id` = `t`.`franchise_id`))) join `stock_order_items` `soi` on((`soi`.`stock_order_id` = `so`.`id`))) group by `f`.`id`,year(`so`.`created_at`),month(`so`.`created_at`) */;
...
-- Dump completed on 2025-08-12  2:16:33
