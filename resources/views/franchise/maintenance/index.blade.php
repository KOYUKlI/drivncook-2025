  ...
  <div class="card-body p-0">
    <table class="data-table">
      ...
      @forelse($records as $rec)
        <tr>
          <td>{{ optional($rec->maintenance_date)->format('Y-m-d') }}</td>
          <td>{{ $rec->truck->name }}</td>
          <td class="text-sm">{{ Str::limit($rec->description, 60) }}</td>
          ...
          <td class="text-center">
            ...
            <x-confirm-delete :name="'delete-maint-' . $rec->id" :action="route('franchise.maintenance.destroy', $rec)" title="Delete maintenance record" :message="'Delete record #' . $rec->id . '?'" />
          </td>
        </tr>
      @empty
        <tr><td colspan="5" class="text-center py-6 text-gray-500">No records yet.</td></tr>
      @endforelse
      </tbody>
    </table>
  </div>
