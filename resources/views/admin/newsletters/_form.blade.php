...
<div class="space-y-4">
  <div>
    <label class="block text-sm font-medium">{{ __('Sujet') }}</label>
  ...
  <div>
    <label class="block text-sm font-medium">{{ __('Programmée le') }}</label>
  ...
  <div>
    <label class="block text-sm font-medium">{{ __('Contenu') }}</label>
  ...
  <div class="flex items-center gap-4">
    <button class="bg-indigo-600 text-white px-4 py-2 rounded">{{ __('Enregistrer') }}</button>
    <a href="{{ route('admin.newsletters.index') }}" class="text-gray-600">{{ __('Annuler') }}</a>
  </div>
</div>
