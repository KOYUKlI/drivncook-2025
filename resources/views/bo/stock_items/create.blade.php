...
@section('content')
    <x-ui.breadcrumbs :items="[
        ...
        ['title' => __('ui.bo.stock_items.create')]
    ]" />

    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('ui.bo.stock_items.create') }}</h1>
        <p class="text-gray-600">{{ __('ui.bo.stock_items.create_subtitle') }}</p>
    </div>

    <div class="bg-white rounded-lg border border-gray-200 shadow-sm">
        <form method="POST" action="{{ route('bo.stock-items.store') }}" class="space-y-6 p-6">
            @csrf
            
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    <x-input-label for="sku" :value="__('ui.bo.stock_items.fields.sku')" />
                    <x-text-input id="sku" name="sku" value="{{ old('sku') }}" required class="mt-1 block w-full" />
                ...
                <div>
                    ...
                    <x-text-input id="unit" name="unit" value="{{ old('unit') }}" required class="mt-1 block w-full" />
                    <x-input-error :messages="$errors->get('unit')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="price_cents" :value="__('ui.bo.stock_items.fields.price_cents')" />
                    <x-text-input id="price_cents" name="price_cents" type="number" min="0" step="1" value="{{ old('price_cents') }}" required class="mt-1 block w-full" />
            ...
            <div class="flex items-center">
                <input id="is_central" name="is_central" type="checkbox" value="1" {{ old('is_central') ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" />
