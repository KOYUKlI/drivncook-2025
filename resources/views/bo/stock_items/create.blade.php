...
@section('content')
<x-ui.page-header title="{{ __('ui.create_stock_item') }}"/>

<form method="POST" action="{{ route('bo.stock-items.store') }}" class="space-y-4 bg-white p-6 border rounded">
  @csrf
  <div>
  ...
  <div>
    ...
    <x-text-input id="price_cents" name="price_cents" type="number" min="0" step="1" value="{{ old('price_cents') }}" required />
    <x-input-error :messages="$errors->get('price_cents')" />
  </div>
  <div class="flex items-center gap-2">
    <input id="is_central" name="is_central" type="checkbox" value="1" {{ old('is_central') ? 'checked' : '' }} />
