...
@section('content')
    <x-ui.breadcrumbs :items="[
        ...
        ['title' => __('ui.bo.warehouses.create')]
    ]" />

    <div class="mb-8">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('ui.bo.warehouses.create') }}</h1>
        <p class="text-gray-600">{{ __('ui.bo.warehouses.create_subtitle') }}</p>
    </div>

    <div class="bg-white rounded-lg border border-gray-200 shadow-sm">
        <form method="POST" action="{{ route('bo.warehouses.store') }}" class="space-y-6 p-6">
            @csrf
            
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    <x-input-label for="code" :value="__('ui.bo.warehouses.fields.code')" />
                    <x-text-input id="code" name="code" value="{{ old('code') }}" required class="mt-1 block w-full" placeholder="WH-001" />
                ...
                <div>
                    ...
                    <x-text-input id="city" name="city" value="{{ old('city') }}" class="mt-1 block w-full" />
                    <x-input-error :messages="$errors->get('city')" class="mt-2" />
                </div>
                
                <div>
                    <x-input-label for="region" :value="__('ui.bo.warehouses.fields.region')" />
                    <x-text-input id="region" name="region" value="{{ old('region') }}" class="mt-1 block w-full" />
                    <x-input-error :messages="$errors->get('region')" class="mt-2" />
                </div>
                
                <div class="sm:col-span-2">
                    <x-input-label for="address" :value="__('ui.bo.warehouses.fields.address')" />
                    <x-textarea-input id="address" name="address" class="mt-1 block w-full" rows="3">{{ old('address') }}</x-textarea-input>
                ...
                <div class="sm:col-span-2">
                    ...
                    <x-textarea-input id="notes" name="notes" class="mt-1 block w-full" rows="3">{{ old('notes') }}</x-textarea-input>
                    <x-input-error :messages="$errors->get('notes')" class="mt-2" />
                </div>
                
                <div>
                    <div class="flex items-center space-x-2">
                        <x-checkbox-input id="is_active" name="is_active" :checked="old('is_active', true)" />
