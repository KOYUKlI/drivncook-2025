...
@section('content')
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm mb-8">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900">Historique d’affectation</h3>
                </div>
                @if($truckModel ?? null)
                    @php
                        $currentOwner = optional(($truckModel->ownerships()->with('franchisee')->whereNull('ended_at')->latest('started_at')->first()))->franchisee;
                    @endphp
                    <div class="text-sm text-gray-600">
                        {{ __('ui.labels.franchisee') }}:
                        <span class="font-semibold text-gray-900">
                            {{ $currentOwner->name ?? __('ui.common.unassigned') }}
                        </span>
                    </div>
                @endif
            </div>
        ...
        <div class="px-6 py-6 overflow-x-auto">
            @php
                $ownerships = ($truckModel ?? null) 
                    ? $truckModel->ownerships()->with('franchisee')->orderByDesc('started_at')->get() 
                    : collect();
            @endphp
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">{{ __('ui.labels.franchisee') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Début</th>
                        <th class="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Fin</th>
                        <th class="px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Statut</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 bg-white">
                    @forelse($ownerships as $own)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ $own->franchisee->name ?? __('ui.common.unassigned') }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{{ optional($own->started_at)->timezone('Europe/Paris')->format('d/m/Y H:i') }}</td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{{ $own->ended_at ? $own->ended_at->timezone('Europe/Paris')->format('d/m/Y H:i') : '—' }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">
                                @if(is_null($own->ended_at))
                                    <span class="inline-flex items-center gap-1.5 px-2 py-0.5 text-xs font-medium rounded-full bg-green-100 text-green-800">
                                        <span class="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                                        Actif
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1.5 px-2 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-gray-800">
                                        Terminé
                                    </span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center text-sm text-gray-500">Aucun historique d’affectation.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm" x-data="{ activeTab: 'deployments' }">
        ...
        <div x-show="activeTab === 'maintenance'" class="p-6">
            ...
            <div class="bg-gradient-to-r from-amber-50 to-yellow-50 rounded-xl border border-amber-200 p-6 mb-6">
                ...
                <form method="POST" action="{{ route('bo.trucks.maintenance.open', $truck['id']) }}" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-6 gap-4">
                    @csrf
